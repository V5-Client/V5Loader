...
internal class ModifyVariableGenerator(
    ...
    override fun attachAnnotation(node: MethodNode, signature: InjectionSignature) {
        node.visitAnnotation(SPModifyVariable::class.descriptorString(), true).apply {
            ...
            visitOptional("slice", modifyVariable.slice?.let(Utils::createSliceAnnotation))
            visitOptional("print", modifyVariable.print)
            visitOptional("ordinal", modifyVariable.ordinal)
            visitOptional("index", modifyVariable.index)
            visitOptional("remap", modifyVariable.remap)
            visitOptional("require", modifyVariable.require)
            visitOptional("expect", modifyVariable.expect)
            visitOptional("allow", modifyVariable.allow)
            visitOptional("constraints", modifyVariable.constraints)
            visitEnd()
        }
    }
