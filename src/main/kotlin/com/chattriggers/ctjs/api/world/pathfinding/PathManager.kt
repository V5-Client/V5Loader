...
object PathManager {
  ...
  private fun flattenBlockPositions(points: List<BlockPos>, yOffset: Int = 0): IntArray {
    val out = IntArray(points.size * 3)
    var idx = 0
    for (point in points) {
      out[idx++] = point.x
      out[idx++] = point.y + yOffset
      out[idx++] = point.z
    }
    return out
  }
  ...
  @JvmStatic
  fun getPathArray(): IntArray = currentPath?.let {
    flattenBlockPositions(it.points, if (it.isFly) 0 else -1)
  } ?: IntArray(0)

  @JvmStatic
  fun getKeyNodesArray(): IntArray = currentPath?.let {
    flattenBlockPositions(it.keyNodes, if (it.isFly) 0 else -1)
  } ?: IntArray(0)

  @JvmStatic
  fun getEtherwarpPathArray(): IntArray = currentEtherwarpPath?.let {
    flattenBlockPositions(it.points)
  } ?: IntArray(0)
