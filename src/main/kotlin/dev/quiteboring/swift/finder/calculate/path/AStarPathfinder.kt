...
import dev.quiteboring.swift.finder.movement.*
...
class AStarPathfinder(
  ...
  private fun createNode(x: Int, y: Int, z: Int): Int {
    ...
    coordToNode.put(PathNode.coordKey(x, y, z), idx)
    return idx
  }
  ...
  private fun seedStartNodes(openSet: PooledHeap, weight: Double) {
    for ((index, point) in startPoints.withIndex()) {
      ...
      val nodeKey = PathNode.coordKey(x, y, z)
  ...
  fun findPath(): Path? {
    ...
    while (openSet.isNotEmpty() && iterations < maxIterations) {
      ...
      for (move in moves) {
        ...
        val neighbourKey = PathNode.coordKey(res.x, res.y, res.z)
